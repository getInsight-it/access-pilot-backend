package it.getinsight.module.user.repository;

import it.getinsight.core.dynamicquery.repository.DynamicNativeQueryRepository;
import it.getinsight.core.dynamicquery.repository.DynamicQueryRepository;
import it.getinsight.module.user.entity.UserEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.Optional;

@Repository
public interface UserRepository extends JpaRepository<UserEntity, Long>, DynamicQueryRepository, DynamicNativeQueryRepository {

    Optional<UserEntity> findByExternalId(String id);

    Optional<UserEntity> findByUsername(String username);

}
